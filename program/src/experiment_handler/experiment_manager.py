...
class ExperimentConfig:
    ...
    phases: List[Dict[str, Any]] = field(default_factory=list)  # Future: phase definitions
...
class ExperimentManager:
    ...
    def __init__(self,
        ...
        threshold = self.config_service.get('data_storage.compression.threshold_bytes', int, 10 * 1024 * 1024)
        max_age = self.config_service.get('data_storage.compression.max_file_age_seconds', int, 24 * 3600)
        ...
        # Event callbacks
        self._state_change_callbacks: List[Callable[[ExperimentState, ExperimentState], None]] = []
    ...
    def _load_experiment_config(self) -> None:
        ...
        try:
            ...
            self.state_messages = exp_config.get('state_output',
                                                 ['Experiment fehlgeschlagen', 'Experiment läuft erfolgreich'])
            # Load new experiments storage paths from data_storage
            storage_cfg = self.config_service.get('data_storage.storage_paths.experiments', dict, {}) or {}
            self.experiments_base_dir = Path(storage_cfg.get('base', 'data/experiments'))
            results_cfg = storage_cfg.get('results', {}) or {}
            self.experiments_results_raw = results_cfg.get('raw', 'results/raw')
            self.experiments_results_processed = results_cfg.get('processed', 'results/processed')
            # Ensure base experiment directory exists
            self.experiments_base_dir.mkdir(parents=True, exist_ok=True)
            debug(f"Experiment config loaded: base_dir={self.experiments_base_dir}, raw={self.experiments_results_raw}, processed={self.experiments_results_processed}, auto_zip={self.auto_zip}")
        except Exception as e:
            error(f"Failed to load experiment configuration: {e}")
            # Fallback defaults
            self.auto_zip = True
            self.naming_pattern = '%Y-%m-%dT%H-%M-%S'
            self.state_messages = ['Experiment fehlgeschlagen', 'Experiment läuft erfolgreich']
    ...
    async def _start_controllers(self, controller_ids: List[str]) -> None:
        ...
        debug(f"Would start controllers: {controller_ids}")
    ...
    async def _save_experiment_summary(self, data_point: ExperimentDataPoint) -> None:
        ...
        try:
            with open(summary_file, 'a', newline='', encoding='utf-8') as f:
                ...
                if write_header:
                    writer.writerow([
                        'timestamp', 'phase', 'sensor_count', 'controller_count',
                        'valid_sensors', 'error_sensors', 'total_data_points'
                    ])
                
                valid_sensors = sum(1 for r in data_point.sensor_readings.values() 
                                  if r.status == SensorStatus.OK)
                error_sensors = sum(1 for r in data_point.sensor_readings.values() 
                                  if r.status != SensorStatus.OK)
    ...
    async def _save_experiment_metadata(self, result: ExperimentResult) -> None:
        ...
        try:
            ...
            metadata = {
                ...
                'configuration': asdict(self._experiment_configs[result.experiment_id]) if result.experiment_id in self._experiment_configs else {},
    ...
    def add_state_change_callback(self, callback: Callable[[ExperimentState, ExperimentState], None]) -> None:
        """Add callback for state changes"""
        self._state_change_callbacks.append(callback)
    
    def add_data_callback(self, callback: Callable[[ExperimentDataPoint], None]) -> None:
    ...
    def get_experiment_statistics(self, experiment_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            'errors_count': result.errors_count,
            'warnings_count': result.warnings_count,
            'result_directory': str(result.result_directory) if result.result_directory else None,
            'compressed_archive': str(result.compressed_archive) if result.compressed_archive else None
        }
...
async def create_and_start_experiment(name: str, 
                                    description: str = "",
                                    duration_minutes: Optional[int] = None,
                                    sensor_ids: Optional[List[str]] = None,
                                    controller_ids: Optional[List[str]] = None) -> Optional[str]:
    """
