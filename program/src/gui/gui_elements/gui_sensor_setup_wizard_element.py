...
class SensorSetupWizardComponent(BaseComponent):
    ...
    def __init__(
        ...
        self._available_filters = {
            'Moving Average': {
                ...
                'parameters': {
                    'window_size': {'type': 'int', 'default': 5, 'min': 2, 'max': 50, 'label': 'Window Size'}
                }
            },
            'Range Validation': {
                ...
                'parameters': {
                    'min_value': {'type': 'float', 'default': None, 'label': 'Minimum Value (optional)'},
                    'max_value': {'type': 'float', 'default': None, 'label': 'Maximum Value (optional)'}
                }
            },
            'Outlier Detection': {
                ...
                'parameters': {
                    'threshold_std': {'type': 'float', 'default': 2.0, 'min': 0.5, 'max': 5.0, 'label': 'Std Dev Threshold'},
                    'min_samples': {'type': 'int', 'default': 10, 'min': 3, 'max': 100, 'label': 'Min Samples'}
                }
            }
        }
    ...
    def _reset_wizard_data(self) -> None:
        ...
        self._wizard_data = {
            ...
            'name': '',
            'type': 'temperature',
            'source': list(SENSOR_REGISTRY.keys())[0] if SENSOR_REGISTRY else 'arduino_tc_board',
            'interface': 'serial',
            'port': 'COM3',
            'channel': 0,
            'poll_interval_ms': 1000,
    ...
    def _render_stepper(self) -> None:
        ...
        with ui.stepper().props("vertical") as stepper:
            ...
            with ui.step("source_config", title="Source & Interface", icon="settings_input_component"):
                self._render_step2()
                with ui.stepper_navigation():
            ...
            with ui.step("review", title="Review & Confirm", icon="check_circle"):
                ...
                with ui.stepper_navigation():
                    ui.button("Previous", on_click=self._stepper.previous)
                    ui.button("Create Sensor", on_click=self._create_sensor).props("color=positive")
                    ui.button("Cancel", on_click=self._close_dialog).props("flat")
    ...
    def _render_step1(self) -> None:
        ...
        with ui.column().classes("gap-4 w-full"):
            ...
            with ui.row().classes("items-center gap-4"):
                ...
                self._step1_elements['sensor_id'] = ui.input(
                    value=self._wizard_data['sensor_id']
                ).props("readonly outlined").classes("flex-1")
                ui.button(
                    icon="refresh", 
                    on_click=self._regenerate_sensor_id
    ...
    def _render_step2(self) -> None:
        ...
        ui.label("Configure sensor source and communication interface").classes("text-lg mb-4")
        
        with ui.column().classes("gap-4 w-full"):
            # Source selection
            with ui.row().classes("items-center gap-4"):
                ui.label("Sensor Source:").classes("w-32 font-semibold")
                self._step2_elements['source'] = ui.select(
                    list(SENSOR_REGISTRY.keys()),
                    value=self._wizard_data['source'],
                    on_change=self._on_source_change
                ).bind_value_to(self._wizard_data, 'source').props("outlined").classes("flex-1")
            
            # Interface type
            with ui.row().classes("items-center gap-4"):
                ui.label("Interface:").classes("w-32 font-semibold")
                self._step2_elements['interface'] = ui.select(
                    ['serial', 'usb', 'ethernet', 'modbus'],
                    value=self._wizard_data['interface'],
                    on_change=self._on_interface_change
                ).bind_value_to(self._wizard_data, 'interface').props("outlined").classes("flex-1")
            
            # Dynamic interface configuration
            self._step2_elements['interface_config'] = ui.column().classes("gap-4 w-full")
            self._update_interface_config()

    def _render_step3(self) -> None:
        """Render step 3: Data processing pipeline setup."""
        ui.label("Configure data processing filters and algorithms").classes("text-lg mb-4")
            ...
            with ui.card().classes("w-full"):
                with ui.card_section():
                    ...
                    with ui.row().classes("items-center gap-4"):
                        ...
                        ui.button(
                            "Add Filter",
                            on_click=lambda: self._add_filter(filter_select.value if filter_select.value else ""),
                            icon="add"
                        ).props("color=primary")
            
            # Current filters
            self._step3_elements['filters_container'] = ui.column().classes("gap-2 w-full")
        ...
        with container:
            if interface in ['serial', 'usb']:
                ...
                with ui.row().classes("items-center gap-4"):
                    ...
                    ).bind_value_to(self._wizard_data, 'port').props("outlined").classes("flex-1")
                
                with ui.row().classes("items-center gap-4"):
                    ui.label("Channel:").classes("w-32 font-semibold")
                    ui.number(
                        min=0, max=15, value=self._wizard_data['channel']
                    ).bind_value_to(self._wizard_data, 'channel').props("outlined").classes("flex-1")
                ...
                with ui.row().classes("items-center gap-4"):
                    ...
                    ).bind_value_to(self._wizard_data, 'baudrate').props("outlined").classes("flex-1")
                
                with ui.row().classes("items-center gap-4"):
                    ui.label("Timeout (s):").classes("w-32 font-semibold")
                    ui.number(
                        min=0.1, max=30.0, step=0.1, value=self._wizard_data['timeout']
                    ).bind_value_to(self._wizard_data, 'timeout').props("outlined").classes("flex-1")
                ...
                with ui.row().classes("items-center gap-4"):
                    ...
                    ).bind_value_to(self._wizard_data, 'unit_id').props("outlined").classes("flex-1")
                ...
                with ui.row().classes("items-center gap-4"):
                    ...
                    ).bind_value_to(self._wizard_data, 'ip_address').props("outlined").classes("flex-1")
            ...
            with ui.row().classes("items-center gap-4"):
                ...
                ).bind_value_to(self._wizard_data, 'poll_interval_ms').props("outlined").classes("flex-1")
    ...
    def _update_filters_display(self) -> None:
        ...
        if not self._wizard_data['filters']:
            with container:
                ui.label("No filters configured").classes("text-gray-500 text-center py-4")
            return
                ...
                with ui.card().classes("w-full"):
                    with ui.card_section():
                        with ui.row().classes("items-center justify-between w-full"):
                            ui.label(f"{filter_config['type']} Filter").classes("font-semibold")
                            ui.button(
                                icon="delete",
                        ...
                        ui.label(filter_def['description']).classes("text-sm text-gray-600 mb-2")
                        ...
                        if filter_config['parameters']:
                            with ui.column().classes("gap-2"):
                                for param_name, param_value in filter_config['parameters'].items():
                                    ...
                                    with ui.row().classes("items-center gap-4"):
                                        ui.label(f"{param_def['label']}:").classes("w-32")
                                        ...
                                        if param_def['type'] == 'int':
                                            ...
                                            ).bind_value_to(filter_config['parameters'], param_name).classes("flex-1")
            ...
            with ui.card().classes("w-full"):
                with ui.card_section():
                    ...
                    with ui.column().classes("gap-1"):
                        ui.label(f"Sensor ID: {self._wizard_data['sensor_id']}").classes("font-mono")
                ...
                with ui.card_section():
                    ...
                    with ui.column().classes("gap-1"):
                        ...
                        if self._wizard_data['interface'] in ['serial', 'usb']:
                            ...
                            ui.label(f"IP Address: {self._wizard_data.get('ip_address', 'Not specified')}")
                            ui.label(f"Port: {self._wizard_data.get('port', 502)}")
                        
                        ui.label(f"Poll Interval: {self._wizard_data['poll_interval_ms']}ms")
              # Data Processing
            with ui.card().classes("w-full"):
                with ui.card_section():
                    ...
                    if self._wizard_data['filters']:
                        with ui.column().classes("gap-2"):
                            for i, filter_config in enumerate(self._wizard_data['filters']):
                                ui.label(f"{i+1}. {filter_config['type']} Filter").classes("font-semibold")
                                if filter_config['parameters']:
    ...
    def _validate_and_next_step2(self) -> None:
        ...
        if self._wizard_data['interface'] in ['serial', 'usb'] and not self._wizard_data['port']:
    ...
    def _create_sensor(self) -> None:
        ...
        try:
            ...
            if self._wizard_data['filters']:
                ...
                for filter_config in self._wizard_data['filters']:
                    ...
                    try:
                        ...
                        info(f"Added algorithm configuration: {algorithm_config['algorithm_id']}")
            ...
            ui.notify(f"Sensor '{sensor_config['name']}' created successfully!", color="positive")
...
class SetupWizardComponent(SensorSetupWizardComponent):
    ...
    def show_dialog(self, start_step: str = "sensors", on_close: Optional[Callable[[], None]] = None) -> None:
