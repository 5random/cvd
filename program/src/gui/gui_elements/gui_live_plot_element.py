...
class LivePlotComponent(TimedComponent):
    ...
    def __init__(
        ...
        sensors_to_display: Optional[List[str]] = None,
    ):
        ...
        history_maxlen = max(
            1,
            int(
                self.plot_config.history_seconds
                * 1000
                / self.plot_config.refresh_rate_ms
            ),
        )
    ...
    def render(self) -> ui.column:
        ...
        with ui.column().classes("w-full h-full") as container:
            ...
            self._update_timer = ui.timer(
                self.plot_config.refresh_rate_ms / 1000, self._update_plot
            )
    ...
    def _render_control_panel(self) -> None:
        ...
        with ui.row().classes("w-full items-center gap-2 mb-2") as self._control_panel:
            ...
            # Clear data button
            ui.button(icon="clear", on_click=self._clear_data).props(
                "flat round"
            ).tooltip("Clear data")
            ...
            # Screenshot button
            ui.button(icon="photo_camera", on_click=self._export_image).props(
                "flat round"
            ).tooltip("Screenshot")
            ...
            ui.icon("fiber_manual_record", size="sm").classes(
                f"text-{status_color}-500"
            )
    ...
    def _create_figure(self) -> go.Figure:
        ...
        fig = go.Figure()
        if self.plot_config.chart_type == "bar":
        ...
        sensors = (
            ...
            if self.sensors_to_display is not None
            else self.sensor_manager.get_all_sensors()
        )
        units = []
        ...
        fig.update_layout(
            xaxis=dict(
                showgrid=self.plot_config.show_grid, gridcolor="lightgray", type="date"
            ),
            yaxis=dict(
    ...
    def _refresh_plot(self) -> None:
                ...
                try:
                    ...
                    if self.plot_config.chart_type == "bar":
                        trace = go.Bar(
                            ...
                            marker_color=series_config.color,
                            hovertemplate=(
        ...
        try:
            ...
            self._plot_element.figure.update_yaxes(
                autorange=self.plot_config.auto_scale
            )
    ...
    def _export_image(self) -> None:
        ...
        try:
            ...
            if dm:
                ...
                path = dm.downloads_dir / f"plot_{timestamp}.png"
            else:
                path = Path(f"plot_{timestamp}.png")
            pio.write_image(self._plot_element.figure, str(path))
            notify_later(f"Screenshot saved: {path}", color="positive")
        except Exception as e:
            error(f"Error exporting image: {e}")
            notify_later("Error saving screenshot", color="negative")
    ...
    def _show_settings(self) -> None:
        ...
        with ui.dialog() as dialog, ui.card():
            ...
            with ui.column().classes("gap-4"):
                ...
                ui.select(
                    options=["line", "bar"],
                    value=self.plot_config.chart_type,
                    label="Diagram Type",
                ).bind_value_to(self.plot_config, "chart_type")

                # Series visibility
                ui.label("Data Series").classes("font-semibold")
                for sensor_id, series_config in self._series_configs.items():
                    ui.checkbox(
                        text=series_config.label, value=series_config.visible
    ...
    def _apply_settings(self, dialog) -> None:
        ...
        for key, data_queue in list(self._data_store.items()):
            ...
            self._data_store[key] = deque(
                data_queue, maxlen=self.plot_config.max_points
            )
        ...
        if self._plot_element:
            self._plot_element.figure = self._create_figure()
            self._refresh_plot()

        dialog.close()

    def add_sensor(
        self, sensor_id: str, label: Optional[str] = None, color: Optional[str] = None
    ) -> None:
