...
class ExperimentConfigDialog:
        ...
        with ui.dialog().props('persistent') as dialog:
            ...
            with ui.card().classes('w-[600px] max-w-[90vw]'):
                ...
                with ui.column().classes('gap-4'):
                    ...
                    ).bind_value_to(self._form_data, 'name').props('outlined').classes('w-full')
                    
                    ui.textarea(
                        'Description', 
                        value=self._form_data['description'],
                        placeholder='Optional description of the experiment purpose and goals'
                    ).bind_value_to(self._form_data, 'description').props('outlined').classes('w-full')
                    ...
                    duration_input = ui.number(
                        ...
                        min=1, max=43200, step=1,  # Up to 30 days
                        placeholder='Leave empty for unlimited duration'
                    ).bind_value_to(self._form_data, 'duration_minutes').props('outlined').classes('w-full')
                    
                    # Bind duration input visibility to checkbox
                    duration_input.bind_visibility_from(self._form_data, 'duration_enabled')
                    
                    # Data Collection Settings
                    ui.separator()
                    ui.label('Data Collection Settings').classes('font-semibold text-lg')
                    
                    ui.number(
                        'Collection Interval (ms)', 
                        value=self._form_data['data_collection_interval_ms'],
                        min=100, max=60000, step=100
                    ).bind_value_to(self._form_data, 'data_collection_interval_ms').props('outlined').classes('w-full')
                    
                    ui.checkbox(
                        'Auto-compress results', 
                        value=self._form_data['auto_compress']
                    ...
                    if self._available_sensors:
                        ...
                        ).bind_value_to(self._form_data, 'sensor_ids').props('outlined use-chips').classes('w-full')
                    else:
                        ui.label('No sensors configured').classes('text-orange-600')
                    
                    # Controller Configuration
                    ui.separator()
                    ui.label('Controller Configuration').classes('font-semibold text-lg')
                    ...
                    if self._available_controllers:
                        ...
                        ).bind_value_to(self._form_data, 'controller_ids').props('outlined use-chips').classes('w-full')
                    else:
                    ...
                    with ui.row().classes('gap-2 justify-end w-full mt-6'):
                        ...
                        ui.button('Create & Start', on_click=self._create_and_start).props('color=positive')
                        ui.button('Create Only', on_click=self._create_only).props('color=primary')
    ...
    def _save_experiment(self, start_immediately: bool = False) -> None:
        ...
        try:
            ...
            duration_minutes = None
            if self._form_data['duration_enabled'] and self._form_data['duration_minutes']:
...
class ExperimentCard(BaseComponent):
            ...
            with ui.card_section():
                ...
                with ui.row().classes('items-center justify-between w-full'):
                    with ui.column().classes('gap-1 flex-1'):
                        ...
                        ui.label(self.experiment_info.experiment_id).classes('text-sm text-gray-500')
                        if self.experiment_info.description:
                            ui.label(self.experiment_info.description).classes('text-sm text-gray-600 mt-1')
                    
                    with ui.column().classes('items-end gap-1'):
                        # Status icon and label
                        with ui.row().classes('items-center gap-2'):
                            self._status_icon = ui.icon('help', size='sm').classes('text-gray-400')
                            self._state_label = ui.label(self.experiment_info.state.value.upper()).classes('text-sm font-semibold')
                ...
                with ui.row().classes('items-center gap-6 w-full mt-3'):
                    with ui.column().classes('gap-1'):
                        ...
                        ui.label(str(self.experiment_info.data_points_collected)).classes('text-sm font-mono')
                        
                    with ui.column().classes('gap-1'):
                        ui.label('Duration').classes('text-xs text-gray-600')
                        duration_text = self._format_duration(self.experiment_info.duration_seconds)
                        ui.label(duration_text).classes('text-sm font-mono')
                        
                    with ui.column().classes('gap-1'):
                        ui.label('Sensors').classes('text-xs text-gray-600')
                        ui.label(str(self.experiment_info.sensor_count)).classes('text-sm font-mono')
                        
                    if self.experiment_info.errors_count > 0:
                        with ui.column().classes('gap-1'):
                            ui.label('Errors').classes('text-xs text-red-600')
                            ui.label(str(self.experiment_info.errors_count)).classes('text-sm font-mono text-red-600')
    ...
    def _render_action_buttons(self) -> None:
        ...
        if state == ExperimentState.IDLE:
            start_btn = ui.button('Start', icon='play_arrow', on_click=self._start_experiment).props('size=sm color=positive')
            self._action_buttons.append(start_btn)
            
        elif state == ExperimentState.RUNNING:
            pause_btn = ui.button('Pause', icon='pause', on_click=self._pause_experiment).props('size=sm color=warning')
            stop_btn = ui.button('Stop', icon='stop', on_click=self._stop_experiment).props('size=sm color=negative')
            self._action_buttons.extend([pause_btn, stop_btn])
            
        elif state == ExperimentState.PAUSED:
            resume_btn = ui.button('Resume', icon='play_arrow', on_click=self._resume_experiment).props('size=sm color=positive')
            stop_btn = ui.button('Stop', icon='stop', on_click=self._stop_experiment).props('size=sm color=negative')
            self._action_buttons.extend([resume_btn, stop_btn])
            
        elif state in [ExperimentState.COMPLETED, ExperimentState.FAILED, ExperimentState.CANCELLED]:
            view_btn = ui.button('View Results', icon='assessment', on_click=self._view_results).props('size=sm')
            self._action_buttons.append(view_btn)
        
        # Cancel button for states that allow it
        if state in [ExperimentState.CONFIGURING, ExperimentState.STARTING, ExperimentState.RUNNING, ExperimentState.PAUSED]:
            cancel_btn = ui.button('Cancel', icon='cancel', on_click=self._cancel_experiment).props('size=sm flat')
    ...
    def _update_status_icon(self) -> None:
        ...
        icon, color = state_config.get(self.experiment_info.state, ('help', 'text-gray-400'))
    ...
    async def _start_experiment(self) -> None:
        ...
        try:
            success = await self.experiment_manager.start_experiment(self.experiment_info.experiment_id)
            if success:
                ui.notify(f'Started experiment: {self.experiment_info.name}', color='positive')
            else:
    ...
    async def _stop_experiment(self) -> None:
        ...
        try:
            ...
            if success:
                ui.notify(f'Stopped experiment: {self.experiment_info.name}', color='info')
            else:
    ...
    async def _pause_experiment(self) -> None:
        ...
        try:
            ...
            if success:
                ui.notify(f'Paused experiment: {self.experiment_info.name}', color='warning')
            else:
    ...
    async def _resume_experiment(self) -> None:
        ...
        try:
            ...
            if success:
                ui.notify(f'Resumed experiment: {self.experiment_info.name}', color='positive')
            else:
    ...
    async def _cancel_experiment(self) -> None:
        ...
        with ui.dialog() as dialog:
            with ui.card():
                ui.label(f'Cancel Experiment: {self.experiment_info.name}').classes('text-lg font-bold')
                ui.label('Are you sure you want to cancel this experiment? Unsaved progress will be lost.').classes('mt-2')
                ...
                with ui.row().classes('gap-2 justify-end mt-4'):
                    ...
                    async def _on_cancel_confirm():
                        try:
                            ...
                            if success:
                                ui.notify(f'Cancelled experiment: {self.experiment_info.name}', color='warning')
                            else:
                                ui.notify('Failed to cancel experiment', color='negative')
                        except Exception as e:
                    ...
                    ui.button('Yes, Cancel', on_click=_on_cancel_confirm).props('color=negative')
    ...
    def _render_current_experiment(self) -> None:
        ...
        if current_experiment_id:
            ...
            if experiment_info:
                ...
                with self._card_container:
                    ui.label('Failed to load experiment information').classes('text-red-600')
        else:
            ...
            with self._card_container:
                with ui.card().classes('w-full border-2 border-dashed border-gray-300'):
                    with ui.card_section().classes('text-center py-8'):
                        ...
                        ui.label('No experiment currently running').classes('text-lg text-gray-600')
                        ui.label('Create and start an experiment to begin data collection').classes('text-sm text-gray-500 mt-2')
    ...
    def _get_experiment_info(self, experiment_id: str) -> Optional[ExperimentInfo]:
        ...
        try:
            ...
            return ExperimentInfo(
                ...
                controller_count=len(config.controller_ids) if config.controller_ids else 0,
    ...
    def _refresh_current_experiment(self) -> None:
        ...
        try:
            ...
            if current_experiment_id and self._current_experiment_card:
                ...
                experiment_info = self._get_experiment_info(current_experiment_id)
                if experiment_info:
                    self._current_experiment_card.update_experiment_info(experiment_info)
            else:
...
class ExperimentHistoryTable(BaseComponent):
    ...
    def __init__(self, experiment_manager: ExperimentManager):
        config = ComponentConfig(
            component_id="experiment_history_table",
            title="Experiment History Table"
        )
        super().__init__(config)
    ...
    def render(self) -> Element:
        ...
        with ui.column().classes('w-full gap-4') as container:
            ...
            columns = [
                ...
                {'name': 'data_points', 'label': 'Data Points', 'field': 'data_points', 'align': 'right'},
            ...
            # Refresh button
            ui.button('Refresh', icon='refresh', on_click=self._load_experiments).props('flat')
...
class ExperimentComponent(BaseComponent):
    ...
    def __init__(self, 
        ...
        if not self.experiment_manager:
            ...
            from src.experiment_handler.experiment_manager import ExperimentManager, set_experiment_manager
        ...
        assert self.experiment_manager is not None, "ExperimentManager must be initialized"
