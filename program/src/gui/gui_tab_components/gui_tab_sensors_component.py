        ...
        with ui.dialog().props('persistent') as dialog:
            ...
            with ui.card().classes('w-96'):
                ...
                with ui.column().classes('gap-4'):
                    ...
                    ui.input('Sensor ID', 
                            value=self._form_data['sensor_id']
                    ).bind_value_to(self._form_data, 'sensor_id').props('outlined').classes('w-full')
                    
                    ui.input('Display Name', 
                            value=self._form_data['name']
                    ).bind_value_to(self._form_data, 'name').props('outlined').classes('w-full')
                    
                    ui.select(['temperature', 'pressure', 'flow', 'other'], 
                             value=self._form_data['type'], 
                             label='Sensor Type'
                    ).bind_value_to(self._form_data, 'type').props('outlined').classes('w-full')
                    
                    # Connection Settings
                    ui.separator()
                    ui.label('Connection Settings').classes('font-semibold')
                    
                    ui.select(list(SENSOR_REGISTRY.keys()), 
                             value=self._form_data['source'], 
                             label='Sensor Source'
                    ).bind_value_to(self._form_data, 'source').props('outlined').classes('w-full')
                    
                    ui.select(['serial', 'usb', 'ethernet'], 
                             value=self._form_data['interface'], 
                             label='Interface'
                    ).bind_value_to(self._form_data, 'interface').props('outlined').classes('w-full')
                    
                    ui.input('Port', 
                            value=self._form_data['port']
                    ).bind_value_to(self._form_data, 'port').props('outlined').classes('w-full')
                    
                    with ui.row().classes('gap-2 w-full'):
                        ui.number('Channel', 
                                 value=self._form_data['channel'],
                                 min=0, max=7
                        ).bind_value_to(self._form_data, 'channel').props('outlined').classes('flex-1')
                        
                        ui.number('Poll Interval (ms)', 
                                 value=self._form_data['poll_interval_ms'],
                                 min=100, max=60000, step=100
                        ).bind_value_to(self._form_data, 'poll_interval_ms').props('outlined').classes('flex-1')
                    ...
                    with ui.row().classes('gap-2 w-full'):
                        ui.number('Baudrate', 
                                 ...
                                 min=9600, max=115200
                        ).bind_value_to(self._form_data, 'baudrate').props('outlined').classes('flex-1')
                        
                        ui.number('Timeout (s)', 
                                 value=self._form_data['timeout'],
                                 min=0.1, max=30.0, step=0.1
                        ).bind_value_to(self._form_data, 'timeout').props('outlined').classes('flex-1')
                    
                    ui.checkbox('Enabled', 
                               value=self._form_data['enabled']
            ...
            if self._is_edit_mode and self._original_sensor_id:
                ...
                if new_id != self._original_sensor_id:
                    ...
                    for existing_id, _ in existing_configs:
                        if existing_id == new_id:
                            ui.notify(f'Sensor ID {new_id} already exists', color='negative')
                            return
                if self.config_service.update_sensor_config(self._original_sensor_id, sensor_config):
                    ui.notify(f'Sensor {self._original_sensor_id} updated successfully', color='positive')
                    success = True
                else:
                ...
                for existing_id, _ in existing_configs:
                    if existing_id == sensor_config['sensor_id']:
                        ui.notify(f'Sensor ID {sensor_config["sensor_id"]} already exists', color='negative')
                        return
                        
                try:
                    self.config_service.add_sensor_config(sensor_config)
                    ui.notify(f'Sensor {sensor_config["sensor_id"]} added successfully', color='positive')
                    success = True
...
class SensorCardComponent(BaseComponent):
            ...
            with ui.card_section():
                ...
                with ui.row().classes('items-center justify-between w-full'):
                    with ui.column().classes('gap-1'):
                        ...
                        ui.label(self.sensor_info.sensor_id).classes('text-sm text-gray-500')
                ...
                with ui.row().classes('items-center justify-between w-full mt-2'):
                    self._timestamp_label = ui.label('Last reading: Never').classes('text-xs text-gray-500')
    ...
    def _update_display(self) -> None:
        ...
        if self._value_label:
            if self.sensor_info.current_value is not None:
                self._value_label.set_text(f"{self.sensor_info.current_value:.2f}°C")
            else:
        ...
        if self._polling_button:
            self._polling_button.set_text('Stop' if self.sensor_info.polling else 'Start')
            self._polling_button.props(f'color={"negative" if self.sensor_info.polling else "positive"}')
    ...
    async def _toggle_polling(self) -> None:
        ...
        try:
            if self.sensor_info.polling:
                ...
                success = await self.sensor_manager.start_sensor(self.sensor_info.sensor_id)
                if success:
                    ui.notify(f'Started polling {self.sensor_info.sensor_id}', color='positive')
                else:
                    ui.notify(f'Failed to start {self.sensor_info.sensor_id}', color='negative')
        except Exception as e:
    ...
    def _delete_sensor(self) -> None:
        ...
        with ui.dialog() as dialog:
            with ui.card():
                ui.label(f'Delete Sensor: {self.sensor_info.sensor_id}').classes('text-lg font-bold')
...
class SensorsComponent(TimedComponent):
    ...
    def _update_status_summary(self) -> None:
        ...
        error_sensors = sum(1 for s in self._sensors_info.values() if s.status and s.status.lower() == 'error')
