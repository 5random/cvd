...
class LogService:
    ...
    def _initialize_logging(self):
        ...
        with self._lock:
            ...
            self.log_level = self.config_service.get('logging.level', default='INFO') or 'INFO'
            log_dir_config = self.config_service.get('logging.log_dir', default='data/logs')
            self.log_dir = Path(log_dir_config) if log_dir_config else Path('data/logs')
            rotation_value = self.config_service.get('logging.log_file_rotation_mb', default=10) or 10
            self.rotation_mb = int(rotation_value)
            retention_value = self.config_service.get('logging.retention_days', default=30) or 30
    ...
    def log_experiment_event(self, event_type: str, experiment_id: str, details: Optional[Dict[str, Any]] = None):
        ...
        if details:
            message += f" - {details}"
        self.info(message, experiment_id=experiment_id, event_type=event_type, details=details)
          # Also log to audit trail
        self.audit(f"EXPERIMENT_{event_type.upper()}", {
            'experiment_id': experiment_id,
            'details': details or {}
        })
    
    def log_sensor_event(self, sensor_id: str, event_type: str, data: Optional[Dict[str, Any]] = None):
    ...
    def log_controller_event(self, controller_id: str, action: str, parameters: Optional[Dict[str, Any]] = None):
        """Log controller actions"""
        message = f"Controller {action}: {controller_id}"
        if parameters:
            message += f" - Parameters: {parameters}"
        self.info(message, controller_id=controller_id, action=action, parameters=parameters)
    
    def log_data_processing(self, process_type: str, input_data: Any, output_data: Any = None, 
        ...
        # Performance logging
        if processing_time:
            self.performance(f"PROCESS_{process_type.upper()}", processing_time, extra_fields)
    ...
    def performance(self, operation: str, duration: float, metadata: Optional[Dict[str, Any]] = None):
        ...
        if 'performance' in self._loggers:
            self._loggers['performance'].info(message, extra={
                'operation': operation,
                'duration': duration,
    ...
    # Audit logging
    def audit(self, action: str, details: Optional[Dict[str, Any]] = None):
        """Log audit events"""
        message = f"{action}"
        if details:
        ...
        if 'audit' in self._loggers:
            self._loggers['audit'].info(message, extra={
                'action': action,
                'details': details or {},
    ...
    # Structured logging
    def structured(self, event: str, data: Optional[Dict[str, Any]] = None, level: str = 'INFO'):
        """Log structured data as JSON"""
        if 'structured' in self._loggers:
            log_method = getattr(self._loggers['structured'], level.lower(), 
...
def performance(operation: str, duration: float, metadata: Optional[Dict[str, Any]] = None):
...
def log_experiment_event(event_type: str, experiment_id: str, details: Optional[Dict[str, Any]] = None):
    """Log experiment event using global instance"""
    get_log_service().log_experiment_event(event_type, experiment_id, details)


def log_sensor_event(sensor_id: str, event_type: str, data: Optional[Dict[str, Any]] = None):
    """Log sensor event using global instance"""
    get_log_service().log_sensor_event(sensor_id, event_type, data)


def log_controller_event(controller_id: str, action: str, parameters: Optional[Dict[str, Any]] = None):
    """Log controller event using global instance"""
    get_log_service().log_controller_event(controller_id, action, parameters)


def log_data_processing(process_type: str, input_data: Any, output_data: Any = None, 
                       processing_time: Optional[float] = None):
    """Log data processing using global instance"""
    get_log_service().log_data_processing(process_type, input_data, output_data, processing_time)
