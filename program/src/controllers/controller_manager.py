...
class ControllerManager:
    ...
    def __init__(self, manager_id: str = "default", max_concurrency: Optional[int] = None):
    ...
    def register_controller(self, controller: ControllerStage) -> None:
        ...
        if controller.controller_id in self._controllers:
            raise ValueError(f"Controller {controller.controller_id} already registered")
    ...
    def unregister_controller(self, controller_id: str) -> bool:
        ...
        # Remove dependencies
        self._dependencies = [dep for dep in self._dependencies 
        ...
        info(f"Unregistered controller {controller_id}")
        return True
    ...
    def add_dependency(self, source_id: str, target_id: str, 
        ...
        info(f"Added dependency: {source_id} -> {target_id}")
    ...
    def _calculate_execution_order(self) -> None:
        ...
        if len(execution_order) != len(self._controllers):
            ...
            raise ValueError(f"Circular dependency detected involving controllers: {remaining}")
        
        self._execution_order = execution_order
        debug(f"Updated execution order: {self._execution_order}")
    ...
    async def start_all_controllers(self) -> bool:
        ...
        for controller_id in self._controllers:
            ...
            if await controller.start():
                ...
                error(f"Failed to start controller {controller_id}")
        
        self._running = success_count > 0
        info(f"Started {success_count}/{len(self._controllers)} controllers")
    ...
    async def stop_all_controllers(self) -> None:
        ...
        info("Stopped all controllers")
    
    async def process_data(self, sensor_data: Dict[str, SensorReading], 
                          metadata: Optional[Dict[str, Any]] = None) -> Dict[str, ControllerResult]:
        ...
        try:
            ...
            error(f"Error in controller processing: {e}")
    ...
    def _prepare_controller_input(self, controller_id: str, 
                                sensor_data: Dict[str, SensorReading],
                                controller_outputs: Dict[str, Any],
                                metadata: Dict[str, Any]) -> ControllerInput:
        ...
        controller_data = {}
        for dep in self._dependencies:
            if dep.target_controller_id == controller_id and dep.source_controller_id in controller_outputs:
    ...
    def _update_processing_stats(self, total_time_ms: float, 
                               results: Dict[str, ControllerResult]) -> None:
        ...
        self._processing_stats = {
            ...
            'individual_stats': {cid: result.processing_time_ms 
    ...
    async def reset_controller(self, controller_id: str) -> bool:
        ...
        try:
            ...
            error(f"Error resetting controller {controller_id}: {e}")
            return False
    ...
    def save_configuration(self, config_path: Path) -> bool:
        ...
        try:
            ...
            info(f"Saved controller configuration to {config_path}")
            return True
