...
class CompressionService:
    ...
    def _load_configuration(self) -> None:
        ...
        try:
            ...
            self._chunk_size = compression_config.get(
            ...
            if self._compression_settings.algorithm not in self.COMPRESSION_ALGORITHMS:
                warning(
                    f"Unsupported compression algorithm: {self._compression_settings.algorithm}. Using gzip."
                )
                self._compression_settings.algorithm = "gzip"

            # Load rotation settings
            rotation_config = self._config_service.get(
                "data_storage.file_rotation", dict, {}
            )
            ...
            self._processed_path = Path(
                storage_paths.get("processed", str(base_data_dir / "processed"))
            )
            self._cache_path = Path(
                storage_paths.get("cache", str(base_data_dir / "cache"))
            )
            # Log directory
            self._log_path = Path(
                storage_paths.get("logs", str(base_data_dir / "logs"))
            )
            # Experiment directory base
            exp_cfg = storage_paths.get("experiments", {}) or {}
            self._experiment_path = Path(
                exp_cfg.get("base", str(base_data_dir / "experiments"))
            )
        ...
        try:
            with self._lock:
                ...
                if output_path is None:
                    ...
                    algorithm_info = self.COMPRESSION_ALGORITHMS[
                        self._compression_settings.algorithm
                    ]
    ...
    def _compress_zip(self, input_path: Path, output_path: Path) -> None:
        ...
        with zipfile.ZipFile(
            ...
            "w",
            compression_level,
            compresslevel=self._compression_settings.level,
        ) as zf:
    ...
    def compress_directory(
        ...
        for file_path in files:
            if file_path.is_file() and not self._is_already_compressed(file_path):
                futures.append(
                    pool.submit_task(self.compress_file, file_path, None, data_type)
                )
        # Collect results
        for fut in futures:
            try:
                ...
                raise
        info(
            f"Compressed {len(compressed_files)} {data_type} files in {directory_path}"
        )
    ...
    def _is_already_compressed(self, file_path: Path) -> bool:
        ...
        return (
            file_path.suffix in compressed_extensions or "compressed" in file_path.parts
        )
    ...
    def rotate_general_data(
        self, data_paths: Optional[List[Union[str, Path]]] = None
    ) -> None:
        """
    ...
    def _rotate_file_if_needed(self, file_path: Path, data_type: str) -> None:
        ...
        try:
            ...
            if (
                ...
                > self._rotation_settings.max_size_mb * 1024 * 1024
            ):
    ...
    def _decompress_standard(
        self, compressed_path: Path, output_path: Path, algorithm: str
    ) -> None:
